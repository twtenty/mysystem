...
public class EmModListener implements ActionListener{

	JFrame EmModListener = new JFrame("员工系统");
	...
	public void actionPerformed(ActionEvent e) {
		// TODO 自动生成的方法存根
		//窗口在电脑正中间
				EmModListener.setBounds((ScreenUtils.getScreenWidth()-WIDTH)/2,
						(ScreenUtils.getScreenHeight()-HEIGHT)/2,WIDTH,HEIGHT);
				
				//设置窗口内容
				BackGroundPanel bgPanel = null;
				try {
					bgPanel = new BackGroundPanel(ImageIO.read(new File("images/15.jfif")));
...
//				btnBox.add(registBtn);
				...
				registBtn.addActionListener(new ActionListener()
		        {
					...
					public void actionPerformed(ActionEvent e) {
						...
						try {
							if((Data.getEm().search(id)!=null||Data.getDe().search(id)!=null)&&id.equals(EmployeeLogin.getId())==false)
							{
							...
							}else if(id.equals("YG")||passwd1.equals(""))
							{
								label.setForeground(Color.red);
								label.setFont(new Font("楷体",Font.BOLD,18));
								...
								now.setWages(EmployeeLogin.getEmm().getWages());
								now.setPasswd(new String(cpField.getPassword()));
								Data.getEm().edit(EmployeeLogin.getEmm(),now);
								EmployeeLogin.setEmm(now);
								EmployeeLogin.setId(now.getId());
								EmployeeLogin.setPasswd(now.getPasswd());
								label.setFont(new Font("楷体",Font.BOLD,18));
								label.setText("修改成功");
							}
				...
				EmModListener.add(bgPanel);
				EmModListener.setVisible(true);
	}
}
