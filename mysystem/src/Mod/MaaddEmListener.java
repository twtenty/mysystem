...
public class MaaddEmListener implements ActionListener{

	@Override
	public void actionPerformed(ActionEvent e) {
		// TODO 自动生成的方法存根
		JFrame MaaddEmListener = new JFrame("员工注册系统");
		
		final int WIDTH = 500;
		final int HEIGHT = 330;
		
		MaaddEmListener.setBounds((ScreenUtils.getScreenWidth()-WIDTH)/2,
				(ScreenUtils.getScreenHeight()-HEIGHT)/2,WIDTH,HEIGHT);
		
		BackGroundPanel bgPanel = null;
		try {
			bgPanel = new BackGroundPanel(ImageIO.read(new File("images/16.jfif")));
...
//		JButton loginBtn = new JButton("登录");
		...
		registBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if(Data.getEm().search(id)!=null||Data.getDe().search(id)!=null)
					{
					...
					}else if(id.equals("YG")==true||passwd1.equals("")==true)
					{
						label.setForeground(Color.red);
						label.setFont(new Font("楷体",Font.BOLD,18));
...
//						System.out.print("\n");
		...
		MaaddEmListener.add(bgPanel);
		MaaddEmListener.setVisible(true);
	}
