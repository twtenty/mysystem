...
public class ManagerRegisterListener implements ActionListener{

	@Override
	public void actionPerformed(ActionEvent e) {
		// TODO 自动生成的方法存根
		String pwd=JOptionPane.showInputDialog("请输入密钥:","");
		if(pwd==null||pwd.equals("123")==false)
		{
			JOptionPane.showMessageDialog(new JFrame(),"密钥错误");
		}
		else
		{
			JFrame ManagerRegisterListener = new JFrame("管理员注册系统");
			
			final int WIDTH = 500;
			final int HEIGHT = 320;
			
			ManagerRegisterListener.setBounds((ScreenUtils.getScreenWidth()-WIDTH)/2,
					(ScreenUtils.getScreenHeight()-HEIGHT)/2,WIDTH,HEIGHT);
			
			BackGroundPanel bgPanel = null;
			try {
				bgPanel = new BackGroundPanel(ImageIO.read(new File("images/7.jfif")));
	...
	//		JLabel yLabel = new JLabel("验  证  码:");
	//		JPasswordField yField = new JPasswordField(10);//输入框
	//		yBox.add(yLabel);
	//		yBox.add(Box.createHorizontalStrut(20));//水平间隔
	//		yBox.add(yField);
	...
	//		JButton loginBtn = new JButton("登录");
			...
			registBtn.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						}else if(id.equals("JL")==true||passwd1.equals("")==true||id.equals("")==true)
						{
							label.setForeground(Color.red);
							label.setFont(new Font("楷体",Font.BOLD,18));
			...
			ManagerRegisterListener.add(bgPanel);
			ManagerRegisterListener.setVisible(true);
		}
